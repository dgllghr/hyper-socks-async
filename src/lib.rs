...
impl Service for Socksv5Connector {
    ...
    fn call(&self, req: Self::Request) -> Self::Future {
        ...
        Box::new(
            TcpStream::connect(&self.proxy_addr, &self.handle)
...
fn write_addr(socket: TcpStream, req: hyper::Uri) -> HandshakeFuture<TcpStream> {
    let host = match req.host() {
        Some(host) => host,
        _ => {
            return Box::new(Err(Error::new(ErrorKind::InvalidInput, "host missing")).into_future());
        }
    };
...
fn do_handshake(
    ...
    req: hyper::Uri,
    creds: Option<(Vec<u8>, Vec<u8>)>,
) -> HandshakeFuture<MaybeHttpsStream<TcpStream>> {
    let is_https = req.scheme() == Some("https");
    let host = match req.host() {
        Some(host) => host.to_string(),
        _ => {
            return Box::new(Err(Error::new(ErrorKind::InvalidInput, "Missing host")).into_future());
        }
    };
    ...
    if is_https {
        Box::new(established.and_then(move |socket| {
            let tls = NativeTlsConnector::builder().build().unwrap();
            tls.connect(&host, socket)
                .map_err(|err| Error::new(ErrorKind::Other, err))
                .map(|socket| MaybeHttpsStream::Https(hyper_tls::TlsStream::from(socket)))
        }))
    } else {
        Box::new(established.map(MaybeHttpsStream::Http))
    }
}
